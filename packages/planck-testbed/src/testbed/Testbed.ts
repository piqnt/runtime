...
export class TestbedInstance implements TestbedInterface {
  ...
  get mouseForce() {
    const config = this.runtime.context.activeTool;
    if (typeof config.mouseForce === "number" && config.mouseForce > 0) {
      return config.mouseForce;
    } else if (typeof config.impulseForce === "number" && config.impulseForce > 0) {
      return -config.impulseForce;
    } else {
      return 0;
    }
  }
  set mouseForce(value: number) {
    const config = this.runtime.context.activeTool;
    if (value > 0) {
      config.mouseForce = value;
      config.impulseForce = 0;
    } else if (value < 0) {
      config.mouseForce = 0;
      config.impulseForce = -value;
    } else {
      config.mouseForce = 0;
      config.impulseForce = 0;
    }
