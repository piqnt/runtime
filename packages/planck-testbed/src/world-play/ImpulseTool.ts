...
import { Vec2, World, type Body, type Vec2Value } from "planck";
...
export class ImpulseTool extends Middleware<ImpulseToolContext> {
  ...
  handlePointerDown = (ev: WorldPointerDown) => {
    ...
    const isActive = typeof config.impulseForce === "number" && config.impulseForce > 0;
    if (!isActive) return;
  ...
  handlePointerUp = (ev: WorldPointerUp) => {
    ...
    const config = this.context.activeTool;
    const impulseForce = typeof config.impulseForce === "number" ? config.impulseForce : 100;

    const force = Vec2.sub(ev.point, this.body.getPosition());
    force.mul(-impulseForce);
