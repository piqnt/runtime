import { Body, MouseJoint, World } from "planck";
...
export class PullTool extends Middleware<PullToolContext> {
  ...
  handlePointerDown = (ev: WorldPointerDown) => {
    ...
    const isActive = config.mouseForce != 0 && +config.mouseForce > 0;
    if (!isActive) return;
    ...
    const mouseForce = typeof config.mouseForce === "number" ? config.mouseForce : 1000;
